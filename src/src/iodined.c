...
/*
does not support ipv6 on windows yet
*/
const static struct sockaddr_storage sockaddr_default = {AF_INET, 0};
#define ADDR_ANY_STR "0.0.0.0"
#else
const static struct sockaddr_storage sockaddr_default = {AF_INET6, 0};
#define ADDR_ANY_STR "::"
#endif
...
static int netmask;
static char ns_ip[INET6_ADDRSTRLEN] = ADDR_ANY_STR;
...
send_raw(int fd, char *buf, int buflen, int user, int cmd, struct query *q)
{
	...
	if (debug >= 2) {
		...
		get_addr(&q->from, buf, sizeof(buf));
...
tunnel_tun(int tun_fd, int dns_fd)
{
	...
	userid = find_user_by_ip_g(header);
	//
	if (userid < 0)
		return 0;
	...
	if(in[0] == 'V' || in[0] == 'v') {
		...
		if (check_user_and_ip(userid, q) != 0) {
			...
			if (read >= 18 && (memcmp(logindata, unpacked+1, 16) == 0)) {
				...
				//yeyue
				/*tempip.s_addr = my_ip;
				inet_pton(AF_INET6, my_ip, &tempip);
				tmp[0] = strdup(inet_ntoa(tempip));
				//tempip.s_addr = users[userid].tun_ip;
				inet_pton(AF_INET6, users[userid].tun_ip, &tempip);
				tmp[1] = strdup(inet_ntoa(tempip));

				read = snprintf(out, sizeof(out), "%s-%s-%d-%d-%s-%s-%d-%d", 
						tmp[0], tmp[1], my_mtu, netmask,"FEC0::1","FEC0::2",1280,64);
				*/
				read = snprintf(out, sizeof(out), "%s-%s-%d-%d", 
						my_ip, users[userid].tun_ip, my_mtu, netmask);
		...
		/* beginning 'I' and the last 0 */
		char reply[MAX(INET6_ADDRSTRLEN,INET_ADDRSTRLEN) + 2];
		...
		//yeyue
		if (atoi(ns_ip) != INADDR_ANY) {
			/* If set, use assigned external ip (-n option) */
			replyaddr = inet_addr(ns_ip);
		//
		} else {
		...
		reply[0] = 'I';
		inet_ntop(AF_INET, &replyaddr, reply + 1, sizeof(reply) - 1);
...
handle_full_packet(int tun_fd, int dns_fd, int userid)
{
	...
	if (ret == Z_OK) {
		...
		touser = find_user_by_ip_g(hdr->ip_dst.s_addr);
...
main(int argc, char **argv)
{
	...
	char listen_ip[INET6_ADDRSTRLEN] = ADDR_ANY_STR;
	//char ns_ip[INET6_ADDRSTRLEN] = ADDR_ANY_STR;
#ifndef WINDOWS32
	struct passwd *pw;
#endif
	int foreground;
	...
	//struct sockaddr_storage my_sock;
	//
	...
	while ((choice = getopt(argc, argv, "vcsfhDu:t:d:m:l:p:n:b:P:z:F:")) != -1) {
		...
		case 'n':
			...
			strncpy(ns_ip, optarg, sizeof(ns_ip));
			break;
		case 'b':
			bind_enable = 1;
	...
	strncpy(my_ip, argv[0], sizeof(my_ip));
	
	if (atoi(my_ip) == INADDR_NONE) {
	...
	if(bind_enable) {
		...
		if (bind_port == port && (atoi(listen_ip) == INADDR_ANY || atoi(listen_ip) == htonl(0x7f000001L))) {
	...
	if (atoi(listen_ip) == INADDR_NONE) {
		warnx("Bad IP address to listen on.");
		usage();
	}
	
	if (atoi(ns_ip) == INADDR_NONE) {
	...
	//set_socket((struct sockaddr_storage*)&sock, my_ip, 0);
	//created_users = init_users(my_ip, netmask);
	//created_users = init_users_v6(my_ip, netmask);
	created_users = init_users_g(my_ip, netmask);
	...
	//my_mtu = mtu;
	my_mtu = get_mtu(my_ip, mtu);
	if (!skipipconfig) {
		if (tun_setip(argv[0], users_get_first_ip(), netmask) != 0 || tun_setmtu(my_mtu) != 0) {
			retval = 1;
			goto cleanup1;
		}
	}
	//yeyue
	/*if ((dnsd_fd = open_dns(port, INADDR_ANY)) == -1) {
		retval = 1;
		goto cleanup2;
	}*/
	...
	//sock.sin6_family = af;
	//sock.sin6_port = port;
	//sock.sin6_addr.s6_addr = htonl(listen_ip);
	set_socket((struct sockaddr_storage*)&sock, listen_ip, port);
	if ((dnsd_fd = open_dns_g((struct sockaddr*)&sock)) == -1) {
		retval = 1;
		goto cleanup2;
	}
	//
	if (bind_enable) {
		if ((bind_fd = open_dns_g((struct sockaddr*)&sockaddr_default)) == -1) {
			retval = 1;
			goto cleanup3;
		}
	}
