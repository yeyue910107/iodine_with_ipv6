...
client_set_nameserver(const char *cp, int port) 
{
	...
	set_socket(&nameserv, cp, port);

	if(nameserv.ss_family == 0)
		errx(1, "error resolving nameserver '%s': %s", cp, getlasterror());

	return nameserv.ss_family;
}
...
{
	static char buf[129];
	get_addr(&raw_serv, buf, sizeof(buf));
	return buf;
}
...
send_query(int fd, char *hostname)
{
	...
	sendto(fd, packet, len, 0, (struct sockaddr*)&nameserv, SOCKET_LEN(nameserv));
...
send_raw(int fd, char *buf, int buflen, int user, int cmd)
{
	...
	sendto(fd, packet, len, 0, (struct sockaddr*)&raw_serv, SOCKET_LEN(raw_serv));
}
			...
			if (read > 0) {
				...
				if (strncmp("LNAK", in, 4) == 0) {
					...
					if ( (tun_setip(client, server, netmask) == 0)
						&& (tun_setmtu(mtu) == 0) ) {
						fprintf(stderr, "Server tunnel is %s\n", server);
						return 0;
					} else {
						errx(4, "Failed to set IP\n");
					}
...
handshake_raw_udp(int dns_fd, int seed)
{
	...
	int r;
	int len;
	char serverstr[MAX(INET6_ADDRSTRLEN,INET_ADDRSTRLEN)];
	...
	for (i=0; running && i<3 ;i++) {
		...
		/*XXX START adjust indent 1 tab back*/
			if (in[0] == 'I') {
				/* Received IP address */
				strncpy(serverstr, in + 1, sizeof(serverstr));
				break;
			}
		/*XXX END adjust indent 1 tab back*/
	...
	set_socket(&raw_serv, serverstr, 53);

	if (raw_serv.ss_family == 0) {
		fprintf(stderr, "Failed to get raw server IP, will use DNS mode.\n");
		return 0;
	}

	fprintf(stderr, "Server is at %s, trying raw login: ", serverstr);
	fflush(stderr);
