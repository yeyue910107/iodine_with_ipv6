...
int
open_dns_g(struct sockaddr* paddr){
	int fd, flag;
	struct sockaddr_storage addr;

	memcpy(&addr, paddr, sizeof(struct sockaddr_storage));

	if ((fd = socket(addr.ss_family, SOCK_DGRAM, IPPROTO_UDP)) < 0) {
		fprintf(stderr, "IPv6 socket failed: got fd %d\n", fd);

		/* IPv6 socket failed, try IPv4 */
		addr.ss_family = AF_INET;
		if((fd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0){
			fprintf(stderr, "IPv4 socket failed: got fd %d\n", fd);
			err(1, "socket");
		}
	}
...
/* To get destination address from each UDP datagram, see iodined.c:read_dns() */
	...
	if(bind(fd, (struct sockaddr*)&addr, SOCKET_LEN(addr)) < 0)
		err(1, "bind");
